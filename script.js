...
function startApp() {
    ...
    if (lastWishTime) {
        ...
        if (timeDiff < COOLDOWN_MS) {
             // If refreshing on cooldown, hide back button
...
function checkCooldownState() {
    ...
    if (lastWishTime) {
        ...
        if (timeDiff < COOLDOWN_MS) {
            ...
            // On fresh load/refresh, ensure we are on cooldown screen if needed
            // But we let startApp or navigation handle the initial screen
            return true; 
        }
    }
    return false;
...
// Navigation Logic specific for Back Button
function goToCooldownFromResult() {
    // Show back button because we came from result
...
const nightColors = [
    { c1: "#1e3a8a", c2: "#0f172a" }, // Blue
    { c1: "#581c87", c2: "#1e1b4b" }, // Purple
    { c1: "#0f766e", c2: "#022c22" }, // Teal
    { c1: "#7f1d1d", c2: "#450a0a" }, // Red
    { c1: "#312e81", c2: "#000000" }  // Indigo
];

function setRandomCardColor() {
    const randomColorPair = nightColors[Math.floor(Math.random() * nightColors.length)];
...
function toggleCard(element) {
    ...
    if (card.classList.contains('flipped')) {
        setTimeout(() => actions.classList.remove('disabled'), 300);
    } else {
...
function downloadCard(event) {
    ...
    // Hide border temporary if needed for cleaner screenshot or keep it
    // For this design, we keep it as it looks good
